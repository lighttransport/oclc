...
MUDADeviceOCL::MUDADeviceOCL(
    MUDADeviceTarget target) : MUDADeviceImpl()
{
...
MUDADeviceOCL::~MUDADeviceOCL()
{
#ifdef HAVE_OPENCL
  {
    ...
    for (it = this->commandQueues.begin();
      it != this->commandQueues.end();
      it++) {
  ...
  bool verbosity)
{
  ...
  cl_platform_id platform_id;
  {
    ...
    if (verbosity) printf("[OCL] Num platforms: %d\n", numPlatforms);
    ...
    if (verbosity) {
      ...
      for (int i = 0; i < numPlatforms; i++) {
        clGetPlatformInfo (platform_ids[i], CL_PLATFORM_NAME, sizeof(buffer), buffer, NULL);
        printf("CL_PLATFORM_NAME:    %s\n",buffer);
        clGetPlatformInfo (platform_ids[i], CL_PLATFORM_PROFILE, sizeof(buffer), buffer, NULL);
        printf("CL_PLATFORM_PROFILE: %s\n",buffer);
        clGetPlatformInfo (platform_ids[i], CL_PLATFORM_VERSION, sizeof(buffer), buffer, NULL);
        printf("CL_PLATFORM_VERSION: %s\n",buffer);
        clGetPlatformInfo (platform_ids[i], CL_PLATFORM_VENDOR, sizeof(buffer), buffer, NULL);
        printf("CL_PLATFORM_VENDOR:  %s\n",buffer);
        clGetPlatformInfo (platform_ids[i], CL_PLATFORM_EXTENSIONS, sizeof(buffer), buffer, NULL);
    ...
    errCode = clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_ALL, max_devices, devices, &num_devices);
    CL_CHECK(errCode);

    assert(num_devices >= 1);

  if (verbosity) printf("[MUDA] [OCL] # of devices = %d\n", num_devices);
  ...
  if (verbosity) {
    ...
    for (int i = 0; i < num_devices; i++) {
      printf("==> Device [%d] ========================\n", i);
      size_t size_ret;
      clGetDeviceInfo( this->devices[i], CL_DEVICE_NAME, sizeof(buffer), &buffer, &size_ret);
      printf("CL_DEVICE_NAME: %s\n",buffer);
      clGetDeviceInfo( this->devices[i], CL_DEVICE_VENDOR, sizeof(buffer), &buffer, (size_t*)buffer);
      printf("CL_DEVICE_VENDOR: %s\n",buffer);
      clGetDeviceInfo( this->devices[i], CL_DEVICE_OPENCL_C_VERSION, sizeof(buffer), &buffer, &size_ret);
      printf("CL_DEVICE_OPENCL_C_VERSION: %s\n",buffer);
      clGetDeviceInfo( this->devices[i], CL_DEVICE_PROFILE , sizeof(buffer), &buffer, &size_ret);
      printf("CL_DEVICE_PROFILE: %s\n",buffer);
      clGetDeviceInfo( this->devices[i], CL_DEVICE_VERSION, sizeof(buffer), &buffer, &size_ret);
      printf("CL_DEVICE_VERSION: %s\n",buffer);
      clGetDeviceInfo( this->devices[i], CL_DRIVER_VERSION, sizeof(buffer), &buffer, &size_ret);
      printf("CL_DRIVER_VERSION: %s\n",buffer);
      clGetDeviceInfo( this->devices[i], CL_DEVICE_EXTENSIONS , sizeof(buffer), &buffer, &size_ret);
      printf("CL_DEVICE_EXTENSIONS: %s\n",buffer);

      cl_uint uival;
      clGetDeviceInfo( this->devices[i], CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(uival), &uival, &size_ret);
      printf("CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS: %d\n",uival);

      cl_bool bval;
      clGetDeviceInfo( this->devices[i], CL_DEVICE_IMAGE_SUPPORT , sizeof(cl_bool), &bval, &size_ret);
      printf("CL_DEVICE_IMAGE_SUPPORT: %d\n", bval);
      if (bval) {
        size_t val;
        clGetDeviceInfo( this->devices[i], CL_DEVICE_IMAGE2D_MAX_WIDTH , sizeof(size_t), &val, &size_ret);
        printf("CL_DEVICE_IMAGE2D_MAX_WIDTH: %d\n", (int)val);
        clGetDeviceInfo( this->devices[i], CL_DEVICE_IMAGE2D_MAX_HEIGHT , sizeof(size_t), &val, &size_ret);
        printf("CL_DEVICE_IMAGE2D_MAX_HEIGHT: %d\n", (int)val);
      }

      {
        cl_ulong uval;
        clGetDeviceInfo( this->devices[i], CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(cl_uint), &uval, &size_ret);
        printf("CL_DEVICE_GLOBAL_MEM_SIZE: %lld\n", uval);
      }

      size_t sz;
      clGetDeviceInfo( this->devices[i], CL_DEVICE_MAX_WORK_GROUP_SIZE , sizeof(size_t), &sz, &size_ret);
      printf("CL_DEVICE_MAX_WORK_GROUP_SIZE: %d\n", (int)sz);

      {
        cl_uint uval;
        clGetDeviceInfo( this->devices[i], CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(cl_uint), &uval, &size_ret);
        printf("CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS: %d\n", (int)uval);
      }

      size_t sz3[3];
      clGetDeviceInfo( this->devices[i], CL_DEVICE_MAX_WORK_ITEM_SIZES, 3*sizeof(size_t), sz3, &size_ret);
      printf("CL_DEVICE_MAX_WORK_ITEM_SIZES: %d, %d, %d\n", (int)sz3[0], (int)sz3[1], (int)sz3[2]);
      }
    }
  }
  ...
  if (verbosity) printf("[MUDA] [OCL] Use device: %d\n", this->currentDeviceID);

  {
    cl_int err;
    context = clCreateContext(
        NULL, 1, &this->devices[this->currentDeviceID], 
        NULL, NULL, &err);
...
MUDAProgram
MUDADeviceOCL::loadKernelSource(
    const char  *filename,
    int nheaders,
    const char **headers,
    const char  *options)
{
  ...
  err = clBuildProgram(program->progObjOCL, 1, &this->devices[this->currentDeviceID], options, NULL, NULL);
  ...
  if(err != CL_SUCCESS) {
    ...
    // Get err str length
	clGetProgramBuildInfo(program->progObjOCL, this->devices[this->currentDeviceID], CL_PROGRAM_BUILD_LOG, 0, NULL, &len);
    buffer.resize(len);

    clGetProgramBuildInfo(program->progObjOCL, this->devices[this->currentDeviceID], CL_PROGRAM_BUILD_LOG, len, &buffer.at(0), NULL);
    //printf("len = %d\n", (int)len);
    printf("err: %s\n", &buffer.at(0));
    exit(1);
  }
...
#else
  {
	  ...
	  cmdq = clCreateCommandQueue(this->context, this->devices[this->currentDeviceID],
                                  0, //CL_QUEUE_PROFILING_ENABLE,
                                  &err);
      if (err != CL_SUCCESS) {
          cout << "[OCL] Failed to create command queue.\n";
...
MUDAProgram MUDADeviceOCL::loadKernelBinary(
    const char *filename)
{
#if HAVE_OPENCL
  assert(this->context != NULL);
  ...
  cout << "[OCL] Load CL kernel: " << path << "\n";
	...
	struct stat st;
	// If this source file does not include <sys/stat.h>, the below line occurs a compile error.
	stat(path, &st);
  ...
  program->progObjOCL = clCreateProgramWithBinary(
        this->context, this->devices.size(), &this->devices[this->currentDeviceID],
        lens, const_cast<const unsigned char**>(bins), NULL, &err);
  CL_CHECK(err);

  err = clBuildProgram(program->progObjOCL, this->devices.size(), &this->devices[this->currentDeviceID], NULL, NULL, NULL);
  delete [] bins[0];
  ...
  if(err != CL_SUCCESS) {
    ...
    clGetProgramBuildInfo(program->progObjOCL, this->devices[this->currentDeviceID], CL_PROGRAM_BUILD_LOG, sizeof(buffer), buffer, &len);
    printf("%s\n", buffer);
    exit(1);
  }
  ...
  //
  {
    for (int i = 0; i < this->devices.size(); i++) {
      ...
      cmdq = clCreateCommandQueue(this->context, this->devices[i], 0,
                                  &err);
      if (err != CL_SUCCESS) {
        cout << "[OCL] Failed to create command queue.\n";
        exit(-1);
...
MUDAKernel
MUDADeviceOCL::createKernel(
    const MUDAProgram program,
    const char *functionName)
{
    ...
    if (err != CL_SUCCESS) {
        ...
        cout << "[OCL] Failed to create kernel. function name = " <<
        functionName << "\n";
    ...
    void   *ptr)
{
    ...
    clEnqueueReadBuffer(this->commandQueues[deviceID],
                        mem->memObjOCL, CL_FALSE, 0,
                        size, ptr, 0, NULL, &event);
    ...
    const void   *ptr)
{
    ...
    cl_int err = clEnqueueWriteBuffer(this->commandQueues[deviceID],
                                      mem->memObjOCL,
                                      CL_TRUE,
                                      0,
                                      size,
...
bool
MUDADeviceOCL::bindMemoryObject(
    MUDAKernel kernel,
    int        argNum,
    MUDAMemory mem)
{
    ...
    cl_int err;
    err = clSetKernelArg(kernel->kernObjOCL, argNum, sizeof(cl_mem), &mem->memObjOCL);
    CL_CHECK(err);
    ...
    size_t localSizeZ)
{
    ...
    local_sizes[2] = 1;
    //printf("dim = %d, sz = %d, %d, %d\n", dimension, sizes[0], sizes[1], sizes[2]);
    //printf("dim = %d, local sz = %d, %d, %d\n", dimension, local_sizes[0], local_sizes[1], local_sizes[2]);
