...
public:
    ...
    //  Function: initialize
    //  Initializes device.
    bool initialize(int platformID = 0, int preferredDeviceID = 0, bool verbosity = false);
    ...
    MUDAProgram loadKernelSource(const char *filename,
        int nheaders, const char **headers, const char *options);
    ...
    //bool writeImage(int ID, MUDAMemory mem, size_t width, size_t height, const void *ptr);
    ...
    bool setArg(MUDAKernel kernel, int argNum,
        size_t size, size_t align, void *arg);
...
public:
    ...
    virtual bool initialize(int platformID, int preferredDeviceID, bool verbosity) = 0;
    ...
    virtual MUDAProgram loadKernelSource(const char *filename,
        int nheaders, const char **headers, const char *options) = 0;
    ...
    virtual MUDAKernel createKernel(const MUDAProgram program, const char *functionName) = 0;
    ...
    //virtual MUDAMemory allocImage(MUDAMemoryType memType, MUDAMemoryAttrib memAttrib,
    ...
    virtual bool bindMemoryObject(MUDAKernel kernel, int argNum, MUDAMemory mem) = 0;

    // Function: setKernelArg
    // Sets any object to the kernel.
    virtual bool setArg(MUDAKernel kernel, int argNum,
        size_t size, size_t align, void *arg) = 0;
    ...
    virtual bool read(int deviceID, MUDAMemory mem, size_t size,
                      void *ptr) = 0;
    ...
    //  This function does not return until actual memory copy is finished
    //  (bloking operation).
    //virtual bool writeImage(int deviceID, MUDAMemory mem, size_t width, size_t height,
    //                   const void *ptr) = 0;
private:
...
public:
    ...
    //  This function does not return until actual memory copy is finished
    //  (bloking operation).
    //bool writeImage(int deviceID, MUDAMemory mem, size_t width, size_t height, const void *ptr);
